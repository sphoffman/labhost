...
RUN sed -i "s/@LABHOST_VERSION@/${LABHOST_VERSION}/g" /etc/motd \
    ...
    && for cmd in \
        ...
        ip-set ipv6-set mac-set mtu-set mtu-all route-add route-del default-gw link-up link-down link-flap \
        path-test port-test ports-scan capture capture-save capture-ring capture-read \
        ...
        iperf-server iperf-client iperf-udp iperf-reverse iperf-bidir iperf-parallel iperf-bind \
        iperf-mcast-server iperf-mcast-send iperf-test mcast-join mcast-send \
        http-server https-server tcp-listen udp-listen dnsmasq-start dnsmasq-stop \
        dhcp-pool-create dhcp-pool-delete dhcp-pool-list dhcp-server-start dhcp-server-stop dhcp-server-status \
        tftp-server lldp-start lldp-show lldp-stop pc-phone-delete pc-phone-status pc-phone-reprovision \
    ...
    && ln -sf /usr/local/lib/labhost/radius-server-start /usr/local/bin/radius-server-start

RUN useradd -m -s /bin/bash lab \
    && echo 'lab ALL=(ALL) NOPASSWD:ALL' >/etc/sudoers.d/lab \
    && chmod 0440 /etc/sudoers.d/lab \
    && mkdir -p /run/sshd /run/labhost /home/lab/.ssh /config /pcaps /scripts \
    && chown -R lab:lab /home/lab /config /pcaps /scripts
